...
function renderPage(code = "", userInput = "", error = "") {
  ...
  <body>
    ...
    <script>
      ...
      const userSecret = document.getElementById("secretInput").value;

      function getNewCode() {
        fetch('/generate-code?secret=' + encodeURIComponent(userSecret))
          .then(res => res.text())
          .then(newCode => {
...
app.post("/", async (req, res) => {
  ...
  try {
    const code = totp.generate(input);
    const findData = await userModel.findOne({ code: input });
    if (!findData) {
      const newUser = new userModel({ code: input });
      await newUser.save();
    }
...
app.get("/generate-code", (req, res) => {
  const input = req.query.secret?.replace(/\s/g, "").toUpperCase() || "";
  try {
    const newCode = totp.generate(input);
    res.send(newCode);
  } catch (err) {
